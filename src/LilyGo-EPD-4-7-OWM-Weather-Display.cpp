...
bool DecodeWeather(WiFiClient& json, String Type) {
  ...
  if (Type == "forecast") {
    ...
    for (byte r = 0; r < max_readings; r++) {
      ...
      if (r < 8) {
        if (WxForecast[r].High > WxConditions[0].High) WxConditions[0].High = WxForecast[r].High;
        if (WxForecast[r].Low < WxConditions[0].Low) WxConditions[0].Low = WxForecast[r].Low;
      }
    }
...
void DisplayTempHumiPressSection(int x, int y) {
  ...
  drawString(x - 30, y + Yoffset, String(WxConditions[0].High, 0) + "° | " + String(WxConditions[0].Low, 0) + "° Hi/Lo", LEFT);
}
...
# 946 "C:/development/Arduino/LilyGo-EPD-4-7-OWM-Weather-Display/src/LilyGo-EPD-4-7-OWM-Weather-Display.ino"
void DrawGraph(int x_pos, int y_pos, int gwidth, int gheight, float Y1Min, float Y1Max, String title, float DataArray[], int readings, boolean auto_scale, boolean barchart_mode) {
